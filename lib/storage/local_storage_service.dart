...
class LocalStorageService {
  ...
  static Future<void> updateStudentData(Map<String, dynamic> studentData) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
  ...
  static Future<void> updateStudentFromProfile(Map<String, dynamic> user) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
  ...
  static Future<LicenseStatus> getLicenseStatus() async {
    ...
    final String key = student['key']?.toString().toLowerCase() ?? "unavailable";
    ...
    final int classId = (rawClassId is int) ? rawClassId : int.tryParse(rawClassId?.toString() ?? "0") ?? 0;
    ...
    // --- 2. TRIAL LOGIC ---
      ...
      if (createdOnStr.isNotEmpty) {
        try {
          ...
          int duration = int.tryParse(student['trail_time']?.toString() ?? "7") ?? 7;
    ...
    if ((currentStatus == LicenseStatus.activated || currentStatus == LicenseStatus.trialing)
        && (classId == 0)) {
